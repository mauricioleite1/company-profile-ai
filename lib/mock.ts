export const mock = [
  {
    company_name: 'McCarren AI',
    service_line:
      'AI-powered proposal automation, RFP response generation, opportunity qualification, knowledge management, enterprise AI agents',
    company_description:
      'McCarren AI is a software company specializing in AI-driven solutions that streamline business development processes. Their platform leverages custom AI agents to generate nuanced and compliant responses to sales requests, RFPs, and questionnaires, enhancing cross-team collaboration and knowledge management. McCarren AI also offers tools for optimizing opportunity matching across public and private markets, ensuring faster and more accurate decision-making.',
    tier1_keywords: [
      'AI proposal automation',
      'RFP response generation',
      'opportunity qualification',
      'knowledge management',
      'enterprise AI agents',
    ],
    tier2_keywords: [
      'sales enablement',
      'business development automation',
      'compliance automation',
      'workflow optimization',
      'AI-driven knowledge base',
      'public sector bidding',
      'private sector opportunities',
      'AWS deployment',
    ],
    emails: '',
    poc: '',
  },
  {
    company_name: 'OpenAI',
    service_line:
      'Artificial intelligence research and deployment, large language models, generative AI tools, AI APIs, enterprise AI solutions',
    company_description:
      'OpenAI is an American artificial intelligence research organization founded in December 2015 and headquartered in San Francisco, California. Its mission is to ensure that artificial general intelligence benefits all of humanity. OpenAI is known for developing advanced AI models such as the GPT series, DALL·E, and ChatGPT, and offers these technologies through APIs and applications to individuals and enterprises worldwide.',
    tier1_keywords: [
      'artificial intelligence',
      'large language models',
      'generative AI',
      'AI research',
      'machine learning',
      'AI deployment',
    ],
    tier2_keywords: [
      'GPT-4',
      'ChatGPT',
      'DALL·E',
      'OpenAI API',
      'AI safety',
      'public benefit corporation',
      'Microsoft partnership',
      'enterprise AI solutions',
    ],
    emails: '',
    poc: '',
  },
  // {
  //   company_name: 'Dasa (Diagnósticos da América S.A.)',
  //   service_line:
  //     'Saúde integrada, medicina diagnóstica, hospitais, genômica, oncologia, telemedicina',
  //   company_description:
  //     'A Dasa é a maior rede de saúde integrada do Brasil e a quinta maior do mundo em medicina diagnóstica. Atua com mais de 50 mil colaboradores e 250 mil médicos parceiros, oferecendo soluções de saúde em todas as etapas do cuidado, como hospitais, telemedicina, genômica, oncologia, vacinação e plataformas digitais.',
  //   tier1_keywords: [
  //     'saúde integrada',
  //     'medicina diagnóstica',
  //     'hospitais',
  //     'genômica',
  //     'oncologia',
  //     'telemedicina',
  //   ],
  //   tier2_keywords: [
  //     'atenção primária',
  //     'coordenação de cuidados',
  //     'plataforma digital',
  //     'pronto atendimento',
  //     'vacinação',
  //     'laboratórios',
  //     'pesquisa clínica',
  //     'Nav',
  //   ],
  //   emails: '',
  //   poc: '',
  // },
  // {
  //   company_name: 'Grupo Sabin',
  //   service_line:
  //     'Saúde integrada, medicina diagnóstica, hospitais, genômica, oncologia, telemedicina',
  //   company_description:
  //     'O Grupo Sabin é uma das maiores redes de medicina diagnóstica do Brasil, com mais de 40 anos de experiência. Oferece serviços de análises clínicas, imagem e saúde ocupacional, além de atuar em áreas como genética e telemedicina.',
  //   tier1_keywords: [
  //     'saúde integrada',
  //     'medicina diagnóstica',
  //     'hospitais',
  //     'genômica',
  //     'oncologia',
  //     'telemedicina',
  //   ],
  //   tier2_keywords: [
  //     'atenção primária',
  //     'coordenação de cuidados',
  //     'plataforma digital',
  //     'pronto atendimento',
  //     'vacinação',
  //     'laboratórios',
  //     'pesquisa clínica',
  //     'Nav',
  //   ],
  //   emails: '',
  //   poc: '',
  // },
  // {
  //   company_name: 'CI&T',
  //   service_line:
  //     'Transformação digital, desenvolvimento ágil de software, consultoria em tecnologia, experiência do usuário, dados e inteligência artificial',
  //   company_description:
  //     'A CI&T é uma multinacional brasileira especializada em transformação digital e desenvolvimento de software. Fundada em 1995 por ex-alunos da Unicamp, a empresa atua globalmente, com presença em países como Estados Unidos, Canadá, Reino Unido, Portugal, China, Colômbia, Japão e Austrália. Com mais de 6.000 colaboradores, a CI&T oferece soluções que abrangem desde estratégia tecnológica até experiência do cliente, dados e IA, atendendo diversos setores, incluindo automotivo, financeiro, saúde, varejo e mídia.',
  //   tier1_keywords: [
  //     'transformação digital',
  //     'desenvolvimento ágil',
  //     'consultoria em tecnologia',
  //     'experiência do usuário',
  //     'inteligência artificial',
  //     'dados',
  //   ],
  //   tier2_keywords: [
  //     'Lean IT',
  //     'engenharia de valor',
  //     'plataforma CI&T FLOW',
  //     'serviços em nuvem',
  //     'UX design',
  //     'automação de processos',
  //     'parcerias estratégicas',
  //     'expansão internacional',
  //   ],
  //   emails: '',
  //   poc: '',
  // },
  // {
  //   company_name: 'Volkswagen do Brasil Ltda.',
  //   service_line:
  //     'Automobile manufacturing, vehicle exports, engine production, mobility solutions',
  //   company_description:
  //     'Volkswagen do Brasil Ltda. is the Brazilian subsidiary of the Volkswagen Group, established in 1953. It has produced over 20 million vehicles in Brazil and has been a market leader for the majority of its existence. The company operates four manufacturing plants across Brazil, producing a range of vehicles including the Polo, Virtus, Nivus, T-Cross, and Amarok. Volkswagen do Brasil also exports vehicles to various countries and has been the largest exporter in the automotive sector in Brazil.',
  //   tier1_keywords: [
  //     'automotive manufacturing',
  //     'vehicle production',
  //     'Volkswagen Group',
  //     'Brazilian automotive industry',
  //     'vehicle exports',
  //   ],
  //   tier2_keywords: [
  //     'São Bernardo do Campo plant',
  //     'Taubaté plant',
  //     'São Carlos engine plant',
  //     'São José dos Pinhais plant',
  //     'flex-fuel vehicles',
  //     'mobility solutions',
  //     'engine production',
  //     'vehicle assembly',
  //   ],
  //   emails: '',
  //   poc: '',
  // },
  // {
  //   company_name: 'McLaren Automotive',
  //   service_line:
  //     'High-performance supercar manufacturing, hybrid and electric vehicle development, motorsport engineering, carbon fiber innovation',
  //   company_description:
  //     "McLaren Automotive is a British luxury automotive manufacturer headquartered at the McLaren Technology Centre in Woking, England. Established in 2010 as a successor to McLaren Cars (founded in 1985), the company specializes in designing and producing high-performance sports cars. McLaren operates independently from the McLaren Formula One team and is renowned for its commitment to innovation, particularly in carbon fiber technology and hybrid powertrains. In April 2025, McLaren merged with British EV start-up Forseven under the ownership of Abu Dhabi's CYVN Holdings, aiming to expand its product range beyond traditional supercars to include electric and potentially SUV models.",
  //   tier1_keywords: [
  //     'supercar manufacturing',
  //     'luxury automotive',
  //     'hybrid vehicles',
  //     'electric vehicles',
  //     'carbon fiber technology',
  //     'motorsport engineering',
  //   ],
  //   tier2_keywords: [
  //     'McLaren Technology Centre',
  //     'McLaren Production Centre',
  //     'McLaren Composites Technology Centre',
  //     'Artura',
  //     '750S',
  //     'W1 hypercar',
  //     'Forseven merger',
  //     'CYVN Holdings',
  //   ],
  //   emails: '',
  //   poc: '',
  // },
];
